...
function reply(int $conv, array $r, ?string $intent = null, ?float $conf = null): void {
    ...
    if (!empty($r['feedback'])) {
        $r['buttons'][] = btn('👍 Helpful', 'feedback', ['rating' => 1, 'message_id' => $id]);
        $r['buttons'][] = btn('👎 Not helpful', 'feedback', ['rating' => 0, 'message_id' => $id]);
    }
...
function menu_reply(int $conv, string $text = 'How can I help you today? Pick a topic or type your question.'): void {
    ...
    foreach (q("SELECT id,name,icon FROM categories WHERE status='active' ORDER BY id")->fetchAll() as $c)
        $b[] = btn($c['icon'] . ' ' . $c['name'], 'category', ['id' => (int)$c['id']]);
    $b[] = btn('❓ Other enquiry', 'other');
...
switch ($action) {
    ...
    case 'category':
        ...
        $b[] = btn('Other', 'other'); $b[] = btn('↩ Back', 'menu');
    ...
    case 'other':
        reply($convId, ['text' => "Type your question below, or contact Student Support directly.", 'buttons' => [btn('🎫 Create Support Ticket', 'ticket'), btn('↩ Main menu', 'menu')]]);
    ...
    case 'message':
        ...
        if ($conf < CONF_HIGH) {
            ...
            reply($convId, ['text' => "Just to be sure, do you mean “{$iss['name']}”?", 'buttons' => [btn('Yes', 'issue', ['id' => (int)$iss['id']]), btn('No, show topics', 'menu'), btn('🎫 Contact Support', 'ticket')]], $intent, $conf);
        }
    ...
    case 'ticket':
        ...
        if ($dup) reply($convId, ['text' => "A ticket for this conversation is already open: {$dup['ticket_number']}.", 'buttons' => [btn('↩ Main menu', 'menu')]]);
        ...
        reply($convId, ['text' => "✅ Ticket $num created. Support staff can see this conversation, so you won't need to explain again. Track it under Support Tickets.", 'buttons' => [btn('↩ Main menu', 'menu')]]);
    ...
    case 'feedback':
        ...
        reply($convId, ['text' => $in['rating'] ? 'Thanks for the feedback! 😊' : 'Sorry about that. You can create a support ticket for further help.', 'buttons' => $in['rating'] ? [btn('↩ Main menu', 'menu')] : ticket_buttons()]);
}
