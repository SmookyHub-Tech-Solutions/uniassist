...
<body class="min-h-screen bg-slate-100 md:grid md:grid-cols-2 font-sans antialiased">
...
<div class="flex items-center justify-center p-6 py-12">
  <div class="w-full max-w-sm">
    <div class="md:hidden flex items-center gap-2.5 mb-6"><div class="h-10 w-10 rounded-xl bg-gradient-to-br from-brand to-teal-600 flex items-center justify-center text-xl">🎓</div><span class="font-extrabold text-navy"><?= APP_NAME ?></span></div>
    <div class="bg-white rounded-3xl shadow-pop border border-slate-200/70 p-7">
    <h2 class="text-xl font-extrabold text-navy tracking-tight"><?= e($heading) ?></h2>
    <p class="text-sm text-slate-500 mb-5 mt-0.5"><?= e($sub) ?></p>
    <?php if ($f = flash()): ?><div role="alert" class="flash mb-4 rounded-xl border-l-4 <?= $f['type']==='success'?'border-green-500':'border-red-500' ?> bg-slate-50 px-3 py-2.5 text-sm <?= $f['type']==='success'?'text-green-700':'text-red-700' ?>"><?= e($f['msg']) ?></div><?php endif;
}
function auth_bottom(): void { echo '</div><p class="text-center text-xs text-slate-400 mt-5">🎓 ' . e(APP_NAME) . '</p></div></div></body></html>'; }
function field(string $name, string $label, string $type = 'text', string $val = ''): void {
    echo '<label class="block text-sm font-medium text-slate-700 mb-3">' . e($label) .
         '<input name="' . e($name) . '" type="' . e($type) . '" value="' . e($val) . '" required class="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand transition"></label>';
}
