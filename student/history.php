...
?>
<h1 class="text-2xl font-bold text-navy mb-5">Conversation History</h1>
<div class="space-y-3">
<?php foreach ($rows as $c): ?>
  <a href="<?= url('student/conversation.php?id=' . $c['id']) ?>" class="block bg-white border border-slate-200 hover:border-brand rounded-2xl p-4">
    <div class="text-xs text-slate-400"><?= e(fmt_date($c['created_at'])) ?><?= $c['cat'] ? ' · ' . e($c['cat']) : '' ?> · <?= (int)$c['n'] ?> messages</div>
    <div class="font-semibold text-navy truncate"><?= e($c['first_q']) ?></div>
  </a>
<?php endforeach; if (!$rows): ?><div class="bg-white border border-dashed border-slate-300 rounded-2xl p-8 text-center text-sm text-slate-500">No conversations yet.</div><?php endif; ?>
</div>
<?php layout_bottom();
