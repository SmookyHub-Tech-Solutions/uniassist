...
function bars(array $rows): void {
  ...
  foreach ($rows as $r) echo '<div class="mb-2"><div class="flex justify-between text-xs mb-0.5"><span>' . e($r['name']) . '</span><span class="text-slate-500">' . (int)$r['n'] . '</span></div><div class="h-2 bg-slate-100 rounded-full"><div class="h-2 bg-teal rounded-full" style="width:' . round($r['n'] / $max * 100) . '%"></div></div></div>';
  if (!$rows) echo '<p class="text-sm text-slate-500">No data yet.</p>';
}
...
<?php foreach ($stats as $l => $n): ?>
  <div class="stat-card bg-white border border-slate-200/70 rounded-2xl p-4 shadow-card flex items-center gap-3">
    <div class="stat-tile h-11 w-11 shrink-0 rounded-xl bg-gradient-to-br from-navy to-slate-700 flex items-center justify-center text-xl"><?= $icons[$l] ?? '📊' ?></div>
    <div class="min-w-0"><div class="text-xs text-slate-500 truncate"><?= e($l) ?></div><div class="text-2xl font-extrabold text-navy tracking-tight"><?= (int)$n ?></div></div>
  </div>
<?php endforeach; ?>
</div>
<h2 class="font-bold text-navy mb-3">Performance</h2>
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
...
<div class="grid lg:grid-cols-2 gap-4 mb-4">
  <div class="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-card"><h3 class="font-bold text-navy mb-3">Most asked categories</h3><?php bars($cats); ?></div>
  <div class="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-card"><h3 class="font-bold text-navy mb-3">Most common issues</h3><?php bars($issues); ?></div>
</div>
<div class="grid lg:grid-cols-2 gap-4">
  <div class="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-card">
    <h3 class="font-bold text-navy mb-3">AI assistant</h3>
    <?php if (GEMINI_API_KEY === ''): ?>
      <div class="flex items-start gap-3"><?= pill('Keyword-matching mode', 'amber') ?></div>
      <p class="text-sm text-slate-500 mt-2">No Gemini key set — the chatbot classifies questions with built-in keyword rules. Set the <code class="text-xs bg-slate-100 rounded px-1.5 py-0.5">GEMINI_API_KEY</code> environment variable (or in <code class="text-xs bg-slate-100 rounded px-1.5 py-0.5">config.php</code>) to enable Gemini intent detection.</p>
    <?php else: ?>
      <div class="flex items-start gap-3"><?= pill('Gemini enabled', 'green') ?></div>
      <p class="text-sm text-slate-500 mt-2">Intent detection via <code class="text-xs bg-slate-100 rounded px-1.5 py-0.5"><?= e(GEMINI_MODEL) ?></code>, with keyword matching as automatic fallback.</p>
    <?php endif; ?>
  </div>
  <div class="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-card">
    <div class="flex items-center justify-between mb-3"><h3 class="font-bold text-navy">Recent activity</h3>
      <a href="<?= url('admin/audit.php') ?>" class="text-xs font-semibold text-brand">Full log →</a></div>
    <?php audit_log_table(); $recent = q('SELECT actor_name,action,created_at FROM audit_log ORDER BY id DESC LIMIT 6')->fetchAll(); ?>
    <?php if (!$recent): ?><p class="text-sm text-slate-500">No activity recorded yet.</p><?php endif; ?>
    <ul class="space-y-2">
    <?php foreach ($recent as $r): ?>
      <li class="flex items-center gap-2.5 text-sm"><span class="h-1.5 w-1.5 shrink-0 rounded-full bg-teal-600"></span>
        <span class="font-medium text-navy truncate"><?= e($r['actor_name'] ?? 'System') ?></span>
        <span class="text-slate-500 truncate"><?= e($r['action']) ?></span>
