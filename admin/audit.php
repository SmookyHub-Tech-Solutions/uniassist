...
$tone = fn(string $a) => str_starts_with($a, 'auth.') ? 'bg-violet-100 text-violet-700'
  : (str_contains($a, 'delete') || str_contains($a, 'login_failed') ? 'bg-red-100 text-red-700'
  : (str_contains($a, 'create') || str_contains($a, 'register') ? 'bg-green-100 text-green-700'
  : (str_contains($a, 'ticket') ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700')));
...
<form class="flex flex-wrap gap-2 mb-5">
  ...
  <a href="<?= url('admin/audit.php') ?>" class="rounded-xl border border-slate-300 bg-white hover:bg-slate-50 px-4 py-2 text-sm font-medium shadow-sm transition">Clear</a>
</form>
<div class="bg-white border border-slate-200/70 rounded-2xl shadow-card overflow-x-auto">
<table class="data-table w-full text-sm"><thead class="bg-slate-50 text-slate-500 text-left"><tr>
  <th class="p-3 font-semibold whitespace-nowrap">Time</th><th class="p-3 font-semibold">Actor</th><th class="p-3 font-semibold">Action</th><th class="p-3 font-semibold">Entity</th><th class="p-3 font-semibold">Details</th><th class="p-3 font-semibold">IP</th>
</tr></thead><tbody>
<?php foreach ($rows as $r): ?>
  <tr class="border-t border-slate-100 hover:bg-slate-50 align-top">
    <td class="p-3 whitespace-nowrap text-slate-500"><?= e(fmt_date($r['created_at'])) ?></td>
    ...
    <td class="p-3 text-slate-500"><?= e(trim(($r['entity'] ?? '') . ($r['entity_id'] !== null ? ' #' . $r['entity_id'] : '')) ?: '—') ?></td>
    <td class="p-3 text-slate-600 max-w-md"><?= e($r['details'] ?? '—') ?></td>
    <td class="p-3 text-slate-400"><?= e($r['ip'] ?? '—') ?></td>
  </tr>
<?php endforeach; if (!$rows): ?><tr><td colspan="6" class="p-6 text-center text-slate-500">No audit entries match these filters.</td></tr><?php endif; ?>
</tbody></table></div>
<?php if ($pages > 1): ?>
<div class="flex items-center gap-2 mt-4 text-sm">
  <?php if ($page > 1): ?><a class="rounded-lg border border-slate-300 bg-white px-3 py-1.5" href="?<?= e($qs(['page' => $page - 1])) ?>">← Prev</a><?php endif; ?>
  <span class="text-slate-500">Page <?= $page ?> of <?= $pages ?></span>
  <?php if ($page < $pages): ?><a class="rounded-lg border border-slate-300 bg-white px-3 py-1.5" href="?<?= e($qs(['page' => $page + 1])) ?>">Next →</a><?php endif; ?>
</div>
<?php endif; ?>
<?php layout_bottom();
