...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($do === 'toggle') {
        ...
        if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($pw) < 8) $err = 'Name, valid email and a password of 8+ characters are required.';
...
<div class="bg-white border border-slate-200/70 rounded-2xl shadow-card overflow-x-auto mb-6">
<table class="data-table w-full text-sm"><thead class="bg-slate-50 text-slate-500 text-left"><tr>
  <th class="p-3 font-semibold">Name</th><th class="p-3 font-semibold">Matric / Email</th><th class="p-3 font-semibold">Role</th><th class="p-3 font-semibold">Status</th><th class="p-3 font-semibold">Actions</th>
</tr></thead><tbody>
<?php foreach ($rows as $r): ?>
  <tr class="border-t border-slate-100 align-top hover:bg-slate-50">
    <td class="p-3 font-medium text-navy"><?= e($r['name']) ?></td><td class="p-3"><?= e($r['matric_number'] ?: '') ?><div class="text-xs text-slate-400"><?= e($r['email']) ?></div></td>
    ...
    <td class="p-3"><div class="flex flex-wrap gap-2 items-center">
      <form method="post"><?= csrf_field() ?><input type="hidden" name="do" value="toggle"><input type="hidden" name="id" value="<?= $r['id'] ?>"><button class="text-brand">Toggle</button></form>
      <form method="post" class="flex gap-1"><?= csrf_field() ?><input type="hidden" name="do" value="reset"><input type="hidden" name="id" value="<?= $r['id'] ?>">
        <input name="password" type="password" placeholder="New password" class="rounded border border-slate-300 px-2 py-1 text-xs w-32"><button class="text-brand text-xs">Reset</button></form>
    </div></td>
  </tr>
<?php endforeach; ?></tbody></table></div>
<h2 class="font-bold text-navy mb-2">Add support staff / admin</h2>
<form method="post" class="bg-white border border-slate-200/70 rounded-2xl shadow-card p-5 flex flex-wrap gap-2"><?= csrf_field() ?><input type="hidden" name="do" value="create">
  <input name="name" placeholder="Full name" required class="<?= $in ?>"><input name="email" type="email" placeholder="Email" required class="<?= $in ?>"><input name="password" type="password" placeholder="Password (8+)" required class="<?= $in ?>">
  <select name="role" class="<?= $in ?>"><option value="support">Support staff</option><option value="admin">Administrator</option></select>
  <button class="rounded-xl bg-brand hover:bg-blue-700 text-white px-4 text-sm font-semibold shadow-sm transition">Create</button></form>
<?php layout_bottom();
